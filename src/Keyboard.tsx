...
export default function Keyboard({ onClick: onClickProps }: KeyboardProps) {
  ...
  return (
    <div className={`flex flex-col bg-wo-keyboard p-8 rounded-lg`}>
      {keyboardKeys.map((keyboardRow, rowIndex) => (
        <div
          key={rowIndex}
          className={`my-1 flex justify-center space-x-1 ${
            rowIndex !== 1 ? "mr-4" : "ml-4"
          }`}
        >
          {keyboardRow.map((key, index) => {
            let styles =
              "text-md rounded font-bold uppercase flex-1 flex justify-center items-center p-4";
            ...
            if (letterState) {
              ...
              styles += " bg-wo-key dark:bg-wo-empty-dark dark:text-white";
            }
...
const keyStateStyles = {
  [LetterState.Miss]: "bg-wo-miss",
  [LetterState.Present]: "bg-wo-present",
  [LetterState.Match]: "bg-wo-success",
  [LetterState.Empty]: "bg-wo-empty",
};
